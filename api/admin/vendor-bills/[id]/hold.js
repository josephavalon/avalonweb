...
export default async function handler(req, res) {
  ...
  try {
    const authed = await requireVendorActor(req, res, ['finance_maker', 'finance_checker'], { aal2: true });
    if (!authed) return;
    requireVendorApEnabled();
    const body = parseVendorBody(req);
    if (typeof body.hold !== 'boolean') throw new Error('hold must be true or false.');
    const result = await authed.db.rpc('set_vendor_bill_hold', {
    ...
    return sendPayOpsError(res, normalizeVendorApError(error), 'The vendor bill hold could not be changed.');
  }
}
