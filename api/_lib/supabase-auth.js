 ...
 * derive the caller's identity + role. This is how the client/admin dashboards
 * authenticate to the API without ever shipping a server secret to the bundle.
 */
 ...
 * { user, role, email, db } on success, or null if unauthenticated /
...
export async function getAuthedUser(req) {
  ...
  return { user, role, email: (user.email || '').trim(), db };
}
