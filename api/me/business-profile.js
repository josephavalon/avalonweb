...
const SECTION = 'business_profile';
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      return res.status(200).json({
        ...
        business_profile: preferences?.business_profile || {},
    ...
    const value = sanitizePreferenceSection(SECTION, body.business_profile);
    ...
    await writeAuditEvent(authed.db, {
      ...
      action: 'nurse_business_profile_updated',
      entityType: 'provider_work_preferences',
      entityId: saved.id,
      payload: { provider_profile_id: provider.id, version: saved.version },
    });
    return res.status(200).json({ ok: true, provider: publicProvider(provider), business_profile: saved.business_profile, version: saved.version, updated_at: saved.updated_at });
  } catch (error) {
    console.warn('[me/business-profile] failed', safeLogContext(error, 'nurse_business_profile_failed'));
    return sendNurseWorkflowError(res, error, 'Could not load or save the nurse business profile.');
  }
}
