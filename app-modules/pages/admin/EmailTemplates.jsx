...
export default function EmailTemplates() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      assertApiResponse(data, { arrays: ['templates'], booleans: ['tableMissing'] }, 'Email templates returned an invalid response.');
      if (!hasObjectRows(data.templates, ['key'])) throw invalidApiResponse('Email templates returned invalid records.');
      ...
      <PageShell embedded subtitle={subtitle}>
        ...
        {!loading && !sourceReady ? (
          <OperationalSourceUnavailable
            title="Email template source unavailable"
            description="Customer email templates could not be verified. No empty template workspace is shown, and editing remains disabled until the live source reconnects."
          />
        ) : loading ? (
