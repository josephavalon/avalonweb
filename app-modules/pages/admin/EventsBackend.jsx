...
const SURFACE = 'av-glass-card rounded-[1.35rem] border border-foreground/[0.12] bg-background/62';
...
function TierEditor({ tier, clinical = false, busy, onSave }) {
  ...
  return (
    <form className={`${SURFACE} p-4`} onSubmit={(event) => { event.preventDefault(); onSave({ ...form, priceCents: Math.round(Number(form.price) * 100), allocation: form.allocation === '' ? null : Number(form.allocation), backOnFloorMinutes: form.backOnFloorMinutes === '' ? null : Number(form.backOnFloorMinutes), clinical }); }}>
      <div className="flex items-start justify-between gap-4"><div><p className="font-body text-[9px] font-bold uppercase tracking-[0.18em] text-foreground/40">{clinical ? 'Avalon clinical entity' : 'Experience commerce'}</p><h3 className="mt-1 font-heading text-3xl uppercase leading-none">{tier?.name || (clinical ? 'New service tier' : 'New admission tier')}</h3></div><LockKeyhole className={`h-4 w-4 ${clinical || form.priceLocked ? 'text-emerald-700' : 'text-foreground/30'}`} /></div>
...
export default function EventsBackend() {
  ...
  return (
    <main className="av-admin-shell min-h-dvh bg-transparent pb-20 text-foreground">
      <header className="av-admin-header sticky top-0 z-40 border-b border-foreground/[0.10] bg-background/64 px-4 py-3 backdrop-blur-3xl">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4"><Link to="/admin" className="flex min-h-11 items-center gap-3"><AvalonMark className="h-7 w-[18px]" /><span className="hidden font-body text-[10px] font-bold uppercase tracking-[0.22em] text-foreground/46 sm:inline">Avalon Admin</span></Link><div className="text-center"><p className="font-heading text-2xl uppercase leading-none">Event Control</p><p className="mt-1 font-body text-[9px] uppercase tracking-[0.2em] text-foreground/40">Approval · pricing · access</p></div><button onClick={async () => { await signOut(); navigate('/admin/login', { replace: true }); }} className="flex h-11 w-11 items-center justify-center rounded-full border border-foreground/[0.14] text-foreground/55" aria-label="Sign out"><LogOut className="h-4 w-4" /></button></div>
      </header>
