...
export default function AdminLogin() {
  useSeo({
    title: 'Admin Sign In - Avalon Vitality',
    description: 'Avalon operations sign-in.',
    path: '/admin/login',
  ...
  // Once signed in, route admins to /admin. Anyone else is bounced out
  // with a clear error — admins are promoted manually in Supabase Studio.
  useEffect(() => {
    if (!user) return;
    if (user.role === 'admin') {
        ...
        <section className="w-full max-w-[440px] rounded-[2rem] border border-foreground/[0.12] bg-foreground/[0.045] p-5 shadow-[0_28px_120px_hsl(var(--foreground)/0.10)] backdrop-blur-2xl sm:p-7">
          <div className="mb-7 flex items-center justify-between gap-4">
            <Link to="/" className="inline-flex min-h-11 flex-col justify-center leading-none transition-opacity hover:opacity-70">
              ...
              <span className="mt-1 block font-body text-[8px] uppercase tracking-[0.38em] text-foreground/58">OPERATIONS</span>
            </Link>
          ...
          {linkSent ? (
            <div className="space-y-5">
              <div className="flex items-start gap-3 rounded-2xl border border-emerald-400/22 bg-emerald-500/[0.08] px-4 py-4 text-emerald-100">
                ...
                <p className="font-body text-sm font-medium leading-relaxed">
                  Sent — check <span className="font-bold">{linkSent}</span> for your sign-in link.
                </p>
              </div>
              <button
                type="button"
                onClick={() => { setLinkSent(''); setEmail(''); }}
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              <div className="space-y-2">
                <label htmlFor="admin-email" className="font-body text-[11px] font-semibold uppercase tracking-[0.18em] text-foreground/58">
                  Admin email
                </label>
                <input
                  id="admin-email"
