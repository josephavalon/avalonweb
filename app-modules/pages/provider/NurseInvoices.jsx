...
export default function NurseInvoices() {
  ...
  const load = useCallback(async () => {
    try { const data = await apiGet('/api/me/nurse-invoices'); setState({ loading: false, error: '', invoices: data.invoices || [] }); }
  ...
  return (
    <main className="min-h-dvh bg-background px-4 py-8 text-foreground">
      <section className="mx-auto max-w-4xl">
        ...
        {state.error ? <p className="mt-5 rounded-2xl bg-red-500/10 p-4 text-red-700">{state.error}</p> : null}
