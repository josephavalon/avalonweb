...
const SHIFT_TIME_FORMATTER = new Intl.DateTimeFormat('en-US', { hour: 'numeric', minute: '2-digit', timeZone: 'America/Los_Angeles' });
const dayKey = (value) => new Date(value).toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' });
const formatDate = (value) => SHIFT_DATE_FORMATTER.format(new Date(value));
...
export default function NurseSchedule() {
  useSeo({ title: 'My Shifts — Avalon Vitality', description: 'Claim, complete, and invoice Avalon shifts.', path: '/provider/shifts', robots: 'noindex, nofollow, noarchive' });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      assertApiResponse(data, { arrays: ['shifts'] }, 'Scheduling returned an invalid shift response.');
      if (!hasObjectRows(data.shifts, ['id', 'version', 'status', 'starts_at', 'ends_at'])) {
        throw invalidApiResponse('Scheduling returned invalid shift records.');
      }
  ...
  const act = async (shift, action) => {
    ...
    try { await apiPost('/api/me/shifts', { shiftId: shift.id, version: shift.version, action }); await load(); }
  ...
  const invoice = (shift) => {
    try {
      ...
      const linked = {
        id: typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : `shift-${Date.now()}`,
      ...
      const shifts = Array.isArray(draft.shifts) ? draft.shifts.filter((row) => row?.date) : [];
      window.sessionStorage.setItem(INVOICE_DRAFT_KEY, JSON.stringify({ ...draft, shifts: [...shifts, linked] }));
      ...
      <main className="min-h-dvh bg-background px-4 py-8 text-foreground">
        <section className="mx-auto max-w-5xl">
          <OperationalSourceUnavailable
            title="Shift source unavailable"
  ...
  return (
    <main className="min-h-dvh bg-background px-4 pb-[calc(6rem+env(safe-area-inset-bottom))] pt-8 text-foreground">
      <section className="mx-auto max-w-5xl">
        ...
        {state.error ? <p role="alert" className="mt-4 rounded-2xl bg-red-500/10 p-4 text-sm text-red-700">{state.error}</p> : null}
        <div className="mt-5 grid gap-3">
          {rows.map((shift) => {
            const assignment = shift.assignment;
            const complete = shift.status === 'completed' || assignment?.status === 'completed';
            const claimable = shift.status === 'open' && (!assignment || ['offered', 'declined'].includes(assignment.status));
            const completable = assignment && ['claimed', 'assigned'].includes(assignment.status)
              && !complete && shift.status !== 'cancelled' && Date.now() >= Date.parse(shift.starts_at);
            return <article key={shift.id} className="rounded-3xl border border-foreground/10 bg-foreground/[0.025] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-semibold">{shift.title}</h2><span className={`rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.12em] ${statusTone(complete ? 'completed' : shift.status)}`}>{complete ? 'completed' : assignment?.status || shift.status}</span></div><p className="mt-2 text-sm text-foreground/60">{formatDate(shift.starts_at)} – {SHIFT_TIME_FORMATTER.format(new Date(shift.ends_at))}</p><p className="mt-2 flex items-center gap-1.5 text-xs text-foreground/45"><MapPin className="h-3.5 w-3.5" />{shift.location_name || shift.service_area || 'Location provided after assignment'}</p>{shift.event ? <p className="mt-1 text-xs text-foreground/45">Event: {shift.event.name}</p> : null}{shift.instructions ? <p className="mt-3 rounded-xl bg-foreground/[0.045] p-3 text-sm text-foreground/60">{shift.instructions}</p> : null}</div><div className="flex flex-wrap gap-2">{filter === 'open' && claimable ? <button type="button" disabled={busy === shift.id} onClick={() => act(shift, 'claim')} className="min-h-11 rounded-full bg-foreground px-5 text-[10px] font-bold uppercase tracking-[0.14em] text-background disabled:opacity-50"><Users className="mr-1 inline h-3.5 w-3.5" />Claim shift</button> : null}{completable ? <button type="button" disabled={busy === shift.id} onClick={() => act(shift, 'complete')} className="min-h-11 rounded-full bg-foreground px-5 text-[10px] font-bold uppercase tracking-[0.14em] text-background disabled:opacity-50"><CheckCircle2 className="mr-1 inline h-3.5 w-3.5" />Complete</button> : null}{complete ? <button type="button" onClick={() => invoice(shift)} className="min-h-11 rounded-full border border-foreground/15 px-5 text-[10px] font-bold uppercase tracking-[0.14em]"><FileText className="mr-1 inline h-3.5 w-3.5" />Start invoice</button> : null}</div></div></article>;
          })}
