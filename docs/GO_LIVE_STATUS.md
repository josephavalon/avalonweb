...
| GL-010 | Post-deploy revenue matrix | flow-drift detected 2026-06-26 — needs test realignment | Re-run against `https://snooches.avalonvitality.co` reveals the test is stale relative to the current booking flow. The "No Add-Ons" label was renamed to "No extras" (fixed in `scripts/revenue-matrix-qa.mjs` this pass) and a new "person count" step now sits between category selection and add-ons selection — the test needs an extra `playwrightClickText('Next')` or `'Add person'` handling there. See `.context/drills/GL-010-revenue-matrix-2026-06-26.log`. Until realigned, no automated revenue-matrix signal. The flow itself appears to work for humans (see 2026-06-18 mobile + stability passes). |
