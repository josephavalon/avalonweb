...
| GL-013 | MFA | release blocker — code candidate ready, production evidence pending | The review candidate uses supported Supabase assurance, centralizes enforcement in the standard Admin/staff helpers and identified custom gates, denies privileged access during forced password rotation, and makes `verify:prod` require both MFA flags. Production remains uncleared until migration 055 is applied, both flags are verified `true`, and named-user enrollment, recovery, AAL1 denial, AAL2 success, and session-revocation drills are captured. Direct Supabase Data API operator policies still need database AAL2 enforcement. |
...
| GL-019 | Cognito front door | shipped, guarded by CI | `snooches.avalonvitality.co` collects zero PHI: intake is a sealed Cognito iframe (`src/components/forms/CognitoFormEmbed.jsx`), 10 PHI-collecting routes bounce to `/start` via `<FrontDoorRedirect>` (`src/lib/frontDoor.js`), and 25 PHI-writing API handlers answer `409 front_door_phi_route_disabled` via `blockFrontDoorPhiRoute()` (`api/_lib/pre-api-guard.js`). Two mutually-exclusive CSP blocks in `vercel.json` allow `https://www.cognitoforms.com` in `frame-src` on the front-door host only, and in `script-src` nowhere. Regression guard `npm run test:front-door` (also folded into `npm run test:launch-blockers`) asserts all of it. Architecture record: `docs/COGNITO_FRONT_DOOR.md`. |
| GL-020 | Cognito + Acuity BAAs signed | pending user action | **Cognito Forms BAA is NOT signed.** The intake form is live but must take **synthetic data only** until it is countersigned — do not route a real patient to `/start` and do not campaign into it. Also confirm the Cognito account is on the **HIPAA Enterprise** plan with **entry encryption ON** and **every field marked Protected**. Acuity BAA remains self-serve via the in-app Powerhouse/Premium link (see GL-012). Both are hard gates on real-PHI launch; the technical controls in GL-019 do not substitute for either. |
