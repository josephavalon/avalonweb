...
- Attio: CRM destination for safe lifecycle/contact fields only. **BAA: not signed; outbound sync is killed by default** — `api/_attio.js` refuses all calls unless `ATTIO_SYNC_ENABLED=true`. Legacy — being replaced by HubSpot (below); scheduled for removal in a follow-up PR.
