...
export default function ProviderLogin() {
  ...
  const onSignIn = async (data) => {
    ...
    if (result.ok) navigate(result.user.redirect || '/provider/dashboard');
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* Ambient gradient — slightly different hue from client portal */}
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] rounded-full bg-accent/[0.04] blur-3xl" />
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between px-6 pt-6 pb-4">
        ...
        <Link to="/members" className="font-body text-xs tracking-[0.2em] uppercase text-foreground/50 hover:text-foreground transition-colors">
          Client Login
        </Link>
      </div>
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="login" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3, ease: EASE }}>
                <div className="mb-8">
                  <div className="flex items-center gap-2 mb-3">
                    <p className="font-body text-[10px] tracking-[0.35em] uppercase text-accent">Provider Portal</p>
                  </div>
                  <h1 className="font-heading text-5xl md:text-6xl text-foreground tracking-wide uppercase leading-[0.95]">
                    Provider<br />Access.
                  </h1>
                  <p className="font-body text-sm text-foreground/50 mt-3">Sign in to manage your visits, shifts, and care protocols.</p>
                </div>
                ...
                <div className="mt-6 rounded-2xl border border-white/[0.07] bg-white/[0.02] px-4 py-3.5">
                  ...
                  <div className="space-y-1">
                    {[['ADMIN001','Admin'],['NURSE001','Nurse']].map(([u,r]) => (
                ...
                <div className="mt-6 pt-6 border-t border-white/8 flex items-center justify-between">
                  <p className="font-body text-[11px] text-foreground/30">Looking for the client portal?</p>
                  <Link to="/members" className="font-body text-[11px] tracking-widest uppercase text-foreground/40 hover:text-foreground transition-colors">
                    Client Login →
                  </Link>
                </div>
              </motion.div>
            ...
            {mode === 'forgot' && (
              <motion.div key="forgot" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3, ease: EASE }}>
                ...
                <div className="mb-8">
                  ...
                  <p className="font-body text-sm text-foreground/50 mt-3">Enter your work email and we'll send a reset link.</p>
                </div>
                <form onSubmit={handleSubmit(onForgot)} className="space-y-4">
                  <div>
                    <label className={labelClass}>Work Email</label>
                    <input type="email" defaultValue={getValues('email')} placeholder="you@avalonvitality.co"
            ...
            {mode === 'sent' && (
              <motion.div key="sent" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, ease: EASE }} className="text-center py-8">
                ...
                <p className="font-body text-sm text-foreground/50 mb-8">If a provider account exists, a reset link is on its way.</p>
