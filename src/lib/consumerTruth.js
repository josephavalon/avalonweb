...
export function buildConsumerTruthLayer({
  ...
  const modules = [
    ...
    module({
      key: 'acuity-slot',
      group: 'Visit',
      label: 'Acuity booking',
      value: appointmentHeld ? 'Held' : 'No slot',
      status: appointmentHeld ? 'Ready' : 'Needed',
      owner: 'Acuity placeholder',
      detail: appointmentHeld ? [booking?.date, booking?.time].filter(Boolean).join(' - ') || booking?.datetime : 'No appointment time is represented locally.',
      nextAction: appointmentHeld ? 'Keep slot or modify before dispatch.' : 'Choose a date and time.',
    ...
    module({
      ...
      owner: 'Care team',
      detail: 'Aftercare, Acuity closeout status, receipt, and rebook prompt unlock after visit close.',
