...
const shellBackground = 'linear-gradient(180deg, transparent, hsl(var(--background) / 0.72) 48%)';
      ...
      <div className={`av-glass-menu mx-auto grid ${maxWidthClass} ${gridClass} gap-0.5 rounded-[16px] border p-0.5`}>
        {items.map((item) => {
          ...
          const content = (
            <>
              <span
                className="relative flex h-6 w-9 items-center justify-center rounded-full transition-colors"
                style={{
              ...
              >
                {Icon && <Icon className="h-[16px] w-[16px]" strokeWidth={primary || active ? 1.95 : 1.55} />}
                {item.badge ? (
              ...
              <span
                className="font-body text-[6.5px] font-bold uppercase leading-none tracking-[0.11em]"
          ...
          return renderAction(
            { ...item, disabled },
            content,
            `flex min-h-[40px] flex-col items-center justify-center gap-0.5 rounded-[14px] px-1 text-center transition-all duration-200 active:scale-[0.96] focus:outline-none focus-visible:ring-2 focus-visible:ring-foreground/35 disabled:pointer-events-none disabled:opacity-40 ${item.className || ''}`,
            itemStyle,
