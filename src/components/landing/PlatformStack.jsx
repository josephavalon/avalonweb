...
import { Brain, BarChart3, Droplet, Truck, Orbit, ChevronRight, Smartphone } from 'lucide-react';
  ...
  return (
    <section id="operating-system" className="py-10 md:py-16 px-4 border-t border-border">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Avalon OS · Mobile — coming soon, now at top of card */}
        <motion.div
          initial={{ opacity: 0, y: 14 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: '-10%' }}
          transition={{ duration: 0.65, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
          className="mx-auto max-w-4xl border border-foreground/20 rounded-xl px-5 md:px-8 py-5 md:py-6 flex items-center gap-4 md:gap-6 mb-8 md:mb-12"
        >
          <div className="shrink-0 flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full border border-accent/50 text-accent">
            <Smartphone className="w-5 h-5 md:w-6 md:h-6" strokeWidth={1.6} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] md:text-xs tracking-[0.35em] text-accent font-body uppercase mb-1">Avalon OS · Mobile · Coming Soon</p>
            <h3 className="font-heading text-lg md:text-2xl text-foreground tracking-wide uppercase leading-tight">iOS & Android</h3>
            <p className="font-body text-xs md:text-sm text-muted-foreground mt-1">Your protocol, your data, your record — in your pocket.</p>
          </div>
        </motion.div>
