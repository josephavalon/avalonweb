...
function IVTierCard({ tier, i, billing }) {
  ...
  return (
    <motion.div
      ...
      className={`flex-shrink-0 w-[calc(100vw-8rem-2rem)] sm:w-[340px] md:w-auto relative border rounded-3xl p-6 flex flex-col ${tier.featured || tier.isMostPopular ? 'border-accent/60 bg-card' : 'border-border bg-card'}`}
    >
...
function SimpleTierCard({ tier, i, billing }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay: i * 0.1, duration: 0.7 }}
      className={`flex-shrink-0 w-[calc(100vw-8rem-2rem)] sm:w-[340px] md:w-auto relative border rounded-3xl p-6 flex flex-col text-center ${tier.featured || tier.isMostPopular ? 'border-accent/60 bg-card' : 'border-border bg-card'}`}
    >
      {(tier.featured || tier.isMostPopular) && <div className="absolute -top-px left-6 right-6 h-px bg-accent" />}
